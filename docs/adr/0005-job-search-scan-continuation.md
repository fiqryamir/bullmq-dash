...
- The UI palette debounces 300ms, filters by state chips, virtualizes the result list, and fetches the continuation with `start` when `deepen` is true. On the home view it searches across every queue; inside a queue's jobs view the same palette is mounted scoped to that queue (hitting the per-queue endpoint and dropping the queue column from its rows).
